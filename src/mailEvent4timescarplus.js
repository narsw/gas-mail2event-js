...
mailEvent4timescarplus.prototype.getEvent = function (message) {
    var dateExp_datetime = /(\d{4}\/\d{2}\/\d{2}\s\d{2}:\d{2})/g;
    var dateExpWithSubString = /(\d{4})\/(\d{2})\/(\d{2})\s(\d{2}):(\d{2})/; // 日時分解
    var reserveNoExp = /(\d{8})/g;

    var body = message.getPlainBody(); //getBodyだとHTMLメールのため
    // 予約番号を取得
    var reserveNo = body.match(reserveNoExp);
    var strReserveNo = ""
    if (reserveNo.length == 1) {
        strReserveNo = reserveNo[0];
    }
